...
const Register = ({ setUser }) => { // <-- Recibe setUser como prop

  // Cambia este URL al endpoint real de tu backend
    ...
    if (
      ...
      const sendData = async () => {
        try {
          ...
          // --> Logueamos al usuario: App.jsx ahora mostrará el Dashboard
          setUser(data)
  ...
  return (
    <article className="w-12/12 h-12/12 flex flex-col justify-center items-center p-8">
      <div className="w-12/12 h-12/12 p-6 space-y-4 md:space-y-6 sm:p-8 dark:bg-gray-800 dark:border-gray-700 flex flex-col">
        ...
        <form className="grid sm:grid-cols-2 grid-cols-1 gap-6" onSubmit={saveForm}>
          ...
          <button type="submit" className="w-full h-fit p-2 rounded-2xl text-white bg-gray-900 hover:bg-slate-800 text-center">
            Registrate
          </button>
          <a href="#" className="text-sm w-full text-center font-medium text-primary-600 hover:underline text-white dark:text-primary-500 ml-2">
            ¿Ya tienes cuenta?
          </a>
        </form>
      </div>
